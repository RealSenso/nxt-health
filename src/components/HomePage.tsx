...
const FEATURES: { kind: SpotKind; icon: React.ElementType; title: string; description: string }[] = [
  {
    ...
    description: 'Unmet needs written by fellow doctors and department heads — problems hospitals actually want solved and will pilot.',
  },
  {
    kind: 'funds', icon: BadgeDollarSign, title: 'Non-dilutive grant funding',
    description: 'Grants tied to specific problems, so you can validate an idea without giving up equity — or your practice.',
  },
  {
    kind: 'roadmap', icon: Route, title: 'Step-by-step roadmaps',
    description: 'A milestone plan written for clinicians, from first interviews to regulatory clearance — every step in plain language.',
  },
  {
...
const PERSONAS = [
  { icon: Stethoscope, title: 'Doctors & clinicians', featured: true, text: "You know the problem firsthand. We give you funding, a roadmap from idea to pilot, and the technical team to build it — no business degree needed." },
  { icon: Wrench, title: 'Engineers & scientists', text: 'Join a doctor-led team and build against a validated clinical need, with the regulatory path already mapped.' },
  { icon: Rocket, title: 'Business co-founders', text: 'Help doctors take their product to market — hospital pilots, reimbursement and fundraising.' },
];

const HOW_IT_WORKS = [
  { icon: UserRound, title: 'Create a free account', text: 'Takes under a minute.' },
  { icon: FileSearch, title: 'Pick a clinical problem', text: 'Save the ones closest to your practice.' },
  { icon: Layers, title: 'Choose what to build', text: 'Device, digital health, diagnostic and more.' },
  { icon: CheckCircle2, title: 'Build your team & pilot', text: 'Bring in engineers, get mentored, reach a hospital pilot.' },
];
    ...
    <div className="min-h-screen w-full bg-[var(--nxt-bg)] text-[var(--nxt-ink)] flex flex-col">
                ...
                <RevealItem key={p.title}>
                  <div className={`h-full rounded-3xl border p-6 ${p.featured ? 'bg-[var(--nxt-mint)] border-[var(--nxt-mint-strong)]/40' : 'bg-[var(--nxt-bg)] border-[var(--nxt-line)]'}`}>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {p.featured && <span className="text-xs font-bold uppercase tracking-wider text-[var(--nxt-mint-deep)]">Who it's for</span>}
                    </div>
        ...
        <section className="nxt-container pb-20">
          <Reveal className="nxt-hero-glow rounded-3xl border border-[var(--nxt-line)] p-8 sm:p-12 flex flex-col md:flex-row items-center gap-8">
            <div className="flex-1 text-center md:text-left">
              <h2 className="font-display text-2xl sm:text-3xl font-bold">Ready to build what you see every shift?</h2>
      ...
      <footer className="border-t border-[var(--nxt-line)] bg-[var(--nxt-surface)]">
        <div className="nxt-container py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-[var(--nxt-ink-soft)]">
          <p><span className="font-display font-bold text-[var(--nxt-ink)]">{PLATFORM_NAME}</span> · Built for doctors, from clinical problem to hospital pilot.</p>
